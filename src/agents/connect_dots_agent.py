    ...
    try:
        ...
        if not match:
            ...
            if clean_json.startswith("```json"):
                clean_json = (
                    clean_json.removeprefix("```json").removesuffix("```").strip()
                )
        ...
        for item in raw_insights:
            ...
            connected_ids = [
                str(i) for i in item.get("item_ids", []) if str(i) in valid_ids
            ]
        ...
        logger.info(
            f"Generated {len(validated_insights)} validated multi-document insights."
        )
