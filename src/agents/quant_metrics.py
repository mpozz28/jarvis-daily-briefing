...
def calculate_market_regime() -> dict[str, dict]:
    ...
    """
    logger.info(
        "Esecuzione Market Quantitative Analysis in corso...",
        extra={"component": "quant_engine"},
    )

    # Asset principali per il briefing (S&P 500, Nasdaq, Bitcoin, Oro)
    assets = {"SPY": "S&P 500", "QQQ": "Nasdaq", "BTC-USD": "Bitcoin", "GLD": "Gold"}
    ...
    try:
        ...
        logger.info(
            f"Analisi quantitativa completata su {len(metrics)} asset.",
            extra={"component": "quant_engine"},
        )
        return metrics
        ...
        logger.error(
            f"Errore critico nel calcolo quantitativo: {e}",
            exc_info=True,
            extra={"component": "quant_engine"},
        )
        return {}
