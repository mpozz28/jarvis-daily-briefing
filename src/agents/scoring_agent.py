...
def calculate_freshness_score(published_date_str: str) -> float:
    ...
    try:
        ...
        article_date = dateutil.parser.parse(str(published_date_str), tzinfos=tzinfos)
...
def score_and_filter_candidates(news_list: list, max_candidates: int = 20) -> list:
    ...
    scored_news = sorted(scored_news, key=lambda x: x['deterministic_score'], reverse=True)
    
    # Ritorna solo i Top N candidati per limitare i token passati all'LLM
    top_candidates = scored_news[:max_candidates]
    logger.info(f"Deterministic Filtering completed. Reduced {len(news_list)} items to Top {len(top_candidates)} candidates.")
