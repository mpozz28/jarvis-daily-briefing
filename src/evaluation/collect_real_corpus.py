...
DEFAULT_FEEDS = {
    ...
    "Guardian_World": {"url": "https://www.theguardian.com/world/rss", "area_hint": "WORLD"},
...
def classify_area(title: str, summary: str, area_hint: str) -> str:
    ...
    if area_hint in {"TECH", "AI/ML", "FINANCE", "SCIENCE", "WORLD", "POLITICS"}:
        return area_hint
...
def collect_corpus(
    ...
    for item in all_items:
        key = item["canonical_url"] or item["id"]
        if key in unique:
            duplicate_count += 1
            continue
        unique[key] = item
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Length of the newest temporal holdout window in days.",
    )
    ...
    print(
        f"Collected {len(corpus)} unique articles from {len(DEFAULT_FEEDS)} feeds. "
        f"Unannotated output: {args.output}"
    )
