...
def run_evaluation():
    ...
    scored_data = score_and_filter_candidates(test_data.copy(), max_candidates=len(test_data), reference_time=REFERENCE_TIME)
    baseline_1_rel = [relevance_map.get(item['id'], 0) for item in scored_data]
    
    # Current: Deterministic + LLM Reranking
    ranked_data = rank_and_filter(test_data.copy(), max_items=20, reference_time=REFERENCE_TIME)
    current_rel = [relevance_map.get(item['id'], 0) for item in ranked_data]
    
    K_list = [3, 5]
    metrics = {"B0_Input": {}, "B1_Deterministic": {}, "Current_LLM": {}}
    systems = [("B0_Input", baseline_0_rel), ("B1_Deterministic", baseline_1_rel), ("Current_LLM", current_rel)]
    ...
    for name in ["B0_Input", "B1_Deterministic", "Current_LLM"]:
        m = metrics[name]
        logger.info(f"{name:<16} | {m['ndcg_3']:.4f} | {m['ndcg_5']:.4f} | {m['p_3']:.4f} | {m['p_5']:.4f} | {m['r_5']:.4f}")
    logger.info("===============================================================")
